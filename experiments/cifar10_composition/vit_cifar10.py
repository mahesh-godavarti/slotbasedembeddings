...
class RoPE2D(nn.Module):
    ...
    with frequencies theta_d = 100^(-d/(rope_dim/2)) per RoPE2D paper (ECCV 2024).
...
class MonoidalAxialPE(nn.Module):
    ...
    def apply_to_attention(self, q, k, patch_positions=None):
        ...
        cos/sin: (n_heads, H*W, n_freq) broadcasts with (B, H, N, half)."""
        B, H, N, D = q.shape
        half = D // 2
...
class Attention(nn.Module):
    def __init__(self, embed_dim, n_heads, pe_module, dropout=0.1):
        super().__init__()
        self.n_heads = n_heads
        self.head_dim = embed_dim // n_heads
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim, n_heads, pe_module, mlp_ratio=4.0, dropout=0.1):
        super().__init__()
        self.norm1 = nn.LayerNorm(embed_dim)
        self.attn = Attention(embed_dim, n_heads, pe_module, dropout=dropout)
...
class ViT(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=3,
                 ...
                 pe_type="learned", n_classes=10, dropout=0.1):
        super().__init__()
        ...
        elif pe_type == "monoidal_axial_perlayer":
            self.pe = MonoidalAxialPE(embed_dim, self.grid_h, self.grid_w, n_heads)  # dummy for forward()
            pe_list = [MonoidalAxialPE(embed_dim, self.grid_h, self.grid_w, n_heads) for _ in range(n_layers)]
            self.pe_layers = nn.ModuleList(pe_list)
        elif pe_type == "joformer_axial_perlayer":
            self.pe = JoFormerAxialPE(embed_dim, self.grid_h, self.grid_w, n_heads)  # dummy for forward()
            pe_list = [JoFormerAxialPE(embed_dim, self.grid_h, self.grid_w, n_heads) for _ in range(n_layers)]
            self.pe_layers = nn.ModuleList(pe_list)
        elif pe_type == "monoidal_perlayer":
            self.pe = MonoidalPE(embed_dim, self.grid_h, self.grid_w, n_heads)  # dummy for forward()
            pe_list = [MonoidalPE(embed_dim, self.grid_h, self.grid_w, n_heads) for _ in range(n_layers)]
            self.pe_layers = nn.ModuleList(pe_list)
        elif pe_type == "joformer_perlayer":
            self.pe = JoFormerPE(embed_dim, self.grid_h, self.grid_w, n_heads)  # dummy for forward()
            pe_list = [JoFormerPE(embed_dim, self.grid_h, self.grid_w, n_heads) for _ in range(n_layers)]
            self.pe_layers = nn.ModuleList(pe_list)
        else:
            raise ValueError(f"Unknown pe_type: {pe_type}")

        # Transformer blocks: per-layer PE if available, otherwise shared
        if hasattr(self, 'pe_layers'):
            self.blocks = nn.ModuleList([
                TransformerBlock(embed_dim, n_heads, self.pe_layers[i], dropout=dropout)
                for i in range(n_layers)
            ])
        else:
            self.blocks = nn.ModuleList([
                TransformerBlock(embed_dim, n_heads, self.pe, dropout=dropout)
...
def rand_augment(x, n_ops=2, magnitude=9):
    """RandAugment: apply n_ops random augmentations at given magnitude.
    x: (B, 3, 32, 32) on GPU, already normalized."""
    B = x.shape[0]
    mag = magnitude / 10.0  # normalize to [0, 1]

    for _ in range(n_ops):
        op = torch.randint(0, 5, (1,)).item()

        if op == 0:  # Brightness
            factor = 1.0 + (torch.rand(B, 1, 1, 1, device=x.device) * 2 - 1) * mag
            x = x * factor
        ...
        elif op == 2:  # Rotation (90 degree increments)
            k = torch.randint(0, 4, (1,)).item()
            if k > 0:
                x = torch.rot90(x, k, dims=(-2, -1))

        elif op == 3:  # Vertical flip
            if torch.rand(1).item() < 0.5:
                x = x.flip(-2)

        elif op == 4:  # Gaussian noise
            noise = torch.randn_like(x) * mag * 0.1
            x = x + noise
...
def augment_batch(x):
    """Apply random horizontal flip, random crop, and random erasing.
    x: (B, 3, 32, 32) on GPU."""
    B = x.shape[0]
    ...
    h_idx = offsets_h[:, None] + torch.arange(32, device=x.device)[None, :]
    w_idx = offsets_w[:, None] + torch.arange(32, device=x.device)[None, :]
    x = x[batch_idx[:, None, None, None],
          torch.arange(3, device=x.device)[None, :, None, None],
          h_idx[:, None, :, None],
          w_idx[:, None, None, :]]

    # RandAugment
    x = rand_augment(x, n_ops=2, magnitude=9)

    # Random erasing (cutout): mask a random 16x16 patch with zeros
    erase_mask = torch.rand(B, device=x.device) < 0.5
    eh = torch.randint(0, 17, (B,), device=x.device)  # 32-16+1=17
    ew = torch.randint(0, 17, (B,), device=x.device)
    for i in range(B):
        if erase_mask[i]:
            x[i, :, eh[i]:eh[i]+16, ew[i]:ew[i]+16] = 0

    return x


def mixup_batch(x, y, n_classes, alpha=0.8):
    """Mixup: blend pairs of images and create soft labels.
    x: (B, 3, 32, 32), y: (B,) long -> (B, 3, 32, 32), (B, n_classes) float."""
    B = x.shape[0]
    device = x.device
    ...
    # Create soft labels
    lam_flat = lam.view(B)
    y_onehot = torch.zeros(B, n_classes, device=device)
    y_onehot.scatter_(1, y.unsqueeze(1), 1.0)
    y_perm_onehot = torch.zeros(B, n_classes, device=device)
    y_perm_onehot.scatter_(1, y[perm].unsqueeze(1), 1.0)
    y_mixed = lam_flat.unsqueeze(1) * y_onehot + (1 - lam_flat.unsqueeze(1)) * y_perm_onehot

    return x_mixed, y_mixed


def get_batch(train_x, train_y, batch_size, n_classes=100, use_mixup=True, label_smoothing=0.0):
    ...
    x = augment_batch(x)
    if use_mixup:
        x, y = mixup_batch(x, y, n_classes)
        # Apply label smoothing to soft labels
        if label_smoothing > 0:
            y = y * (1 - label_smoothing) + label_smoothing / n_classes
    elif label_smoothing > 0:
        # Convert hard labels to smooth soft labels
        y_smooth = torch.full((batch_size, n_classes), label_smoothing / n_classes, device=y.device)
        y_smooth.scatter_(1, y.unsqueeze(1), 1.0 - label_smoothing + label_smoothing / n_classes)
        y = y_smooth
    return x, y
...
def main():
    ...
    parser.add_argument("--pe_type", choices=["learned", "rope2d", "joformer_old", "monoidal_axial", "joformer_axial", "monoidal_axial_perlayer", "joformer_axial_perlayer", "rope2dv2", "monoidal", "joformer", "joformer_fixed", "monoidal_perlayer", "joformer_perlayer"],
                        default="monoidal")
    ...
    parser.add_argument("--warmup_epochs", type=int, default=0,
                        help="Warmup epochs at warmup_lr")
    parser.add_argument("--warmup_lr", type=float, default=2e-4,
                        help="Learning rate during warmup phase")
    parser.add_argument("--high_epochs", type=int, default=0,
                        help="High LR epochs after warmup")
    parser.add_argument("--high_lr", type=float, default=1e-3,
                        help="Learning rate during high phase")
    ...
    warmup_total = args.warmup_epochs + args.high_epochs
    # Cosine decay starts AFTER warmup+high phase
    cosine_iters = (args.epochs - warmup_total) * iters_per_epoch if warmup_total > 0 else total_iters
    scheduler = (torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(cosine_iters, 1))
                 if args.cosine_decay else None)
    criterion = nn.CrossEntropyLoss()

    # Piecewise LR schedule: warmup -> high -> cosine/base
    def get_epoch_lr(epoch):
        if args.warmup_epochs > 0 and epoch <= args.warmup_epochs:
            return args.warmup_lr
        elif args.high_epochs > 0 and epoch <= args.warmup_epochs + args.high_epochs:
            return args.high_lr
        else:
    ...
    ckpt_path = os.path.join(args.checkpoint_dir,
                             f"{args.pe_type}_d{args.embed_dim}_l{args.n_layers}.pt")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        # Set LR: warmup/high phase overrides scheduler
        epoch_lr = get_epoch_lr(epoch)
        ...
        for _ in range(iters_per_epoch):
            x, y = get_batch(train_x, train_y, args.batch_size, n_classes=n_classes,
                             label_smoothing=args.label_smoothing)
            optimizer.zero_grad()
            out = model(x)
            # Mixup produces soft labels (B, n_classes), use soft cross-entropy
            if y.dim() == 2:
                loss = (-y * F.log_softmax(out, dim=1)).sum(dim=1).mean()
                correct += out.argmax(1).eq(y.argmax(1)).sum().item()
            else:
                loss = criterion(out, y)
                correct += out.argmax(1).eq(y).sum().item()
            loss.backward()
            optimizer.step()
            if scheduler and epoch_lr is None:
                scheduler.step()
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"Epoch {epoch}: lr={cur_lr:.1e}, train_loss={train_loss:.4f}, train_acc={train_acc:.1f}%, "
