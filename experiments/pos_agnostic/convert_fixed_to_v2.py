#!/usr/bin/env python3
"""Convert a joformer_fixed checkpoint to joformer2 (split_angles) format.

The joformer2 model starts as joformer_fixed (identity angle deviations)
...
def convert(src_path, dst_path):
    ...
    # Build joformer2 model with split_angles
    n_embed = cfg['n_embed']
    model = GPTModel(
        cfg['vocab_size'], n_embed, cfg['n_layers'], cfg['n_heads'],
        cfg['block_size'], dropout=0.1, attn_config='joformer2',
    ...
    # Save with joformer2 config
    new_cfg = dict(cfg)
    new_cfg['attn_config'] = ['joformer2'] * cfg['n_layers']
    torch.save({
    ...
    print(f"  Dest config: joformer2 x {cfg['n_layers']}, split_angles=True")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--src', type=str, required=True, help='joformer_fixed checkpoint')
    parser.add_argument('--dst', type=str, required=True, help='output joformer2 checkpoint')
