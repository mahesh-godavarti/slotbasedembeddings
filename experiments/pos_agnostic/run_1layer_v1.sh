...
# monoidal (datadep v1, cumsum, no V rotation)
echo "Running monoidal (1 layer)..."
/home/ubuntu/exp8/venv/bin/python train.py \
    $COMMON --models monoidal \
    --checkpoint_dir checkpoints/1layer_monoidal \
    --gpu 3

# joformer (datadep v1, cumsum + V rotation)
echo "Running joformer (1 layer)..."
/home/ubuntu/exp8/venv/bin/python train.py \
    $COMMON --models joformer \
    --checkpoint_dir checkpoints/1layer_joformer \
    --gpu 3

# det_qk (fixed signs per token × fixed rope freq)
echo "Running det_qk (1 layer)..."
/home/ubuntu/exp8/venv/bin/python train.py \
    $COMMON --angle_hidden_mult 1 --models shared_det_qk \
    --checkpoint_dir checkpoints/1layer_det_qk \
    --gpu 3

# detb_qk (random Bernoulli signs × fixed rope freq)
echo "Running detb_qk (1 layer)..."
/home/ubuntu/exp8/venv/bin/python train.py \
    $COMMON --angle_hidden_mult 1 --models shared_detb_qk \
