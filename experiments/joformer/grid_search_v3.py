#!/usr/bin/env python3
"""Grid search v3: lr=2e-4, 200k iters, softmax, vocab=8000, full wiki (streaming).
Uses train_wiki_streaming.py with memmap data loading.
Preprocesses once, then runs all configs from the same preprocessed data."""
...
VOCAB_SIZE = 8000
DATA_DIR = Path(__file__).parent / "data_v8k"
...
MODELS = ["roformer", "joformer_fixed", "joformer_learned", "joformer_projected"]


def result_file(n_embed, n_layers):
    return RESULTS_DIR / f"joformer_gridv3_n{n_embed}_L{n_layers}.json"


def log_file(n_embed, n_layers):
    return LOG_DIR / f"joformer_gridv3_n{n_embed}_L{n_layers}.log"


def preprocess():
    """Preprocess full wiki with vocab=8000. One-time cost."""
    bin_path = DATA_DIR / "wiki_tokens.bin"
    if bin_path.exists():
        print(f"Preprocessed data already exists in {DATA_DIR}", flush=True)
        return True

    print(f"Preprocessing full wiki with vocab={VOCAB_SIZE}...", flush=True)
    cmd = [
    ...
    log = LOG_DIR / "joformer_gridv3_preprocess.log"
    ...
    if proc.returncode != 0:
        print(f"  Preprocessing FAILED (exit code {proc.returncode})", flush=True)
        return False

    print(f"  Preprocessing complete.", flush=True)
    return True
...
def print_summary(all_results):
    print("\n" + "=" * 110, flush=True)
    print("GRID SEARCH V3 SUMMARY — vocab=8000, full wiki, lr=2e-4, 200k iters", flush=True)
    print("Best Val PPL (iteration) / Final Val PPL", flush=True)
    print("=" * 110, flush=True)
    print(f"{'Config':<12} {'roformer':>20} {'jo_fixed':>20} {'jo_learned':>20} {'jo_projected':>20}", flush=True)
    print("-" * 110, flush=True)
    ...
    for key in sorted(all_results.keys()):
        ...
        for m in MODELS:
            if m in best and best[m]["best_val_ppl"] is not None:
                ...
                vals.append("--")
        print(f"{key:<12} {vals[0]:>20} {vals[1]:>20} {vals[2]:>20} {vals[3]:>20}", flush=True)
    print("=" * 110, flush=True)


def main():
    # Step 1: Preprocess full wiki (one-time)
    if not preprocess():
        print("Aborting: preprocessing failed.", flush=True)
        sys.exit(1)
    print(flush=True)
    ...
    print(f"Grid search v3: {total} configs, 4 models each", flush=True)
    print(f"vocab={VOCAB_SIZE}, full wiki, lr=2e-4, 200k iters, softmax", flush=True)
    ...
    combined_path = RESULTS_DIR / "joformer_gridv3_results.json"
