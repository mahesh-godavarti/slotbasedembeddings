#!/usr/bin/env python3
"""Grid search v5: Full wiki (streaming/memmap), lr=2e-4, 200k iters, softmax,
vocab=32000, 3 models (no joformer_learned).
...
DATA_DIR = Path(__file__).parent / "data_full_v32k"
...
def result_file(n_embed, n_layers):
    return RESULTS_DIR / f"joformer_gridv5_n{n_embed}_L{n_layers}.json"


def log_file(n_embed, n_layers):
    return LOG_DIR / f"joformer_gridv5_n{n_embed}_L{n_layers}.log"
...
def print_summary(all_results):
    print("\n" + "=" * 90, flush=True)
    print("GRID SEARCH V5 SUMMARY (Full Wiki, vocab=32000) — Best Val PPL (iteration) / Final Val PPL", flush=True)
    print("=" * 90, flush=True)
...
def main():
    # Phase 1: Preprocess
    print("=" * 60, flush=True)
    print("PHASE 1: Preprocessing full wiki (vocab=32000)", flush=True)
    print("=" * 60, flush=True)
    if not preprocess():
    ...
    print(f"Grid search v5 (full wiki, vocab=32000): {total} configs, {len(MODELS)} models each", flush=True)
    ...
    combined_path = RESULTS_DIR / "joformer_gridv5_results.json"
