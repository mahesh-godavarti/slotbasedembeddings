#!/usr/bin/env python3
"""Grid search v2: lr=2e-4, 200k iters, softmax, all 4 models.
...
def result_file(n_embed, n_layers):
    return RESULTS_DIR / f"joformer_gridv2_n{n_embed}_L{n_layers}.json"


def log_file(n_embed, n_layers):
    return LOG_DIR / f"joformer_gridv2_n{n_embed}_L{n_layers}.log"
...
def print_summary(all_results):
    print("\n" + "=" * 110, flush=True)
    print("GRID SEARCH V2 SUMMARY — Best Val PPL (iteration) / Final Val PPL", flush=True)
...
def main():
    ...
    print(f"Grid search v2: {total} configs, 4 models each", flush=True)
    print(f"lr=2e-4, 200k iters, softmax, no cosine decay", flush=True)
    ...
    combined_path = RESULTS_DIR / "joformer_gridv2_results.json"
