#!/usr/bin/env python3
"""Grid search v3: Full wiki (streaming/memmap), lr=2e-4, 200k iters, softmax, all 4 models.
...
DATA_DIR = Path(__file__).parent / "data_full"
...
def preprocess():
    ...
    cmd = [
        ...
        # no --wiki_lines means process all lines
    ]
...
def print_summary(all_results):
    print("\n" + "=" * 110, flush=True)
    print("GRID SEARCH V3 SUMMARY (Full Wiki) — Best Val PPL (iteration) / Final Val PPL", flush=True)
...
def main():
    # Phase 1: Preprocess
    print("=" * 60, flush=True)
    print("PHASE 1: Preprocessing full wiki", flush=True)
    print("=" * 60, flush=True)
    if not preprocess():
    ...
    print(f"Grid search v3 (full wiki): {total} configs, 4 models each", flush=True)
