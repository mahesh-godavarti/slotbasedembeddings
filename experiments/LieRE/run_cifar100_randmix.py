"""CIFAR-100 runner for random-mix rotation (2x2 block rotations + random orthogonal mixing)."""
import sys
...
from models.rope_vit_randmix import RoPEViT as RoPEViTRandMix
import argparse
...
def main():
    ...
    # Create LiereImageClassification and swap in randmix model
    ...
    # Replace with random-mix model
    model.model = RoPEViTRandMix(
        image_size=32,
        patch_size=[4, 4],
        num_classes=100,
        dim=cfg["dim"],
    ...
    n_rot_params = sum(p.numel() for name, p in model.named_parameters() if 'freqs' in name)
    n_mix_params = sum(p.numel() for name, p in model.named_parameters() if 'mix_matrices' in name)
    print(f"Random-mix rotation, rotate_v={args.rotate_v}, n_rounds={args.n_rounds}")
    print(f"Total params: {n_params:,} (rotation freqs: {n_rot_params:,}, fixed mix matrices: {n_mix_params:,})")
