"""CIFAR-100 runner for axial butterfly rotation (commutative + full mixing)."""
import sys
...
from models.rope_vit_axial_butterfly import RoPEViT as RoPEViTAxialButterfly
import argparse
...
def main():
    ...
    # Create LiereImageClassification and swap in axial butterfly model
    ...
    # Replace with axial butterfly model
    model.model = RoPEViTAxialButterfly(
        image_size=32,
        patch_size=[4, 4],
        num_classes=100,
        dim=cfg["dim"],
    ...
    print(f"Axial butterfly rotation, rotate_v={args.rotate_v}, n_rounds={args.n_rounds}")
    print(f"Total params: {n_params:,} (rotation freqs: {n_rot_params:,})")
