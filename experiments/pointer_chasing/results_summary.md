...
#### D=23 K=5 training curve (in progress)

Note: D=23 iter counter is inflated by 15K due to batch size change mid-run (30K iters at batch=16, then switched to batch=32 with flash attention at iter 30K). Real token-equivalent = iter - 15K.
...
D=23 crossed over roformer at 25K-equiv and has stayed ahead since. Gap oscillates between -0.18 and -0.53, averaging **-0.38**. Not the 6-7 PPL advantage hoped for — the shared-weight iteration mechanism provides only a modest edge over separate-weight roformer at near FLOP parity (284C² vs 288C²).
