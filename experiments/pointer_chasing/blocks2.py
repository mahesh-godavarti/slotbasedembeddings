...
class RoFormerAttention(nn.Module):
    def __init__(self, n_embed, block_size, dropout, use_softmax=False, n_head=1, no_rope=False, window=None):
        super().__init__()
        ...
        self.window = window
        # Causal mask (optionally windowed)
    ...
    def forward(self, x):
        ...
        if not self.no_rope:
            # Standard RoPE: absolute positions, no flip (cacheable)
        ...
        if self.use_softmax and H > 1 and self.window is None:
            # Flash attention (full causal, no window)
            q = q.view(B, H, T, D)
            k = k.view(B, H, T, D)
            v = v.view(B, H, T, D)
...
class RoFormerBlock(nn.Module):
    def __init__(self, n_embed, block_size, dropout, use_softmax=False, n_head=1, no_rope=False, window=None):
        super().__init__()
        self.sa_head = RoFormerAttention(n_embed, block_size, dropout, use_softmax, n_head=n_head, no_rope=no_rope, window=window)
