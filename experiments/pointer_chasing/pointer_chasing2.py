...
class PointerChasingDataset:
    ...
    def __init__(self, n_keys=8, n_values=16, n_hops=3, n_show=None, permutation=False, seed=None):
        ...
        # 2: EQUALS token (=)
        # 3: LEVEL_SEP token (|)
        # 4 .. 4+n_keys*n_hops-1: per-level key tokens (A0,B0,..,A1,B1,..,)
        # 4+n_keys*n_hops .. 4+n_keys*n_hops+n_values-1: value tokens
        self.PAD = 0
        self.QUERY = 1
        self.EQUALS = 2
        self.LEVEL_SEP = 3
        self.key_offset = 4
        self.value_offset = 4 + n_keys * n_hops
        self.vocab_size = 4 + n_keys * n_hops + n_values
    ...
    def generate_example(self):
        ...
        # Query section for base level: Q key answer (target at key position)
        for k in pick_shown_keys(base_required):
            tokens.append(self.QUERY)
            targets.append(IGNORE)
            tokens.append(self.key_token(k, level=0))
            targets.append(self.value_token(base_table[k]))
            tokens.append(self.value_token(base_table[k]))  # answer as input
            targets.append(IGNORE)
        tokens.append(self.LEVEL_SEP)
        targets.append(IGNORE)

        # Index levels: no targets in table, targets only in query sections
        for level in range(1, self.n_hops):
            ...
            # Query section: Q key answer (target at key position)
            for k in pick_shown_keys(level_required):
                ...
                tokens.append(self.value_token(resolved))  # answer as input
                targets.append(IGNORE)
            tokens.append(self.LEVEL_SEP)
            targets.append(IGNORE)

        # Final query: no answer token (this is the actual prediction)
        tokens.append(self.QUERY)
        targets.append(IGNORE)
...
class LookAheadD1Sequential(nn.Module):
    ...
    def __init__(self, vocab_size, n_embed, block_size, n_head=4, dropout=0.0, z_residual=False):
        super().__init__()
...
class TransformerBaseline(nn.Module):
    ...
    def __init__(self, vocab_size, n_embed, n_layers, block_size, n_head=4, dropout=0.0, no_rope=False):
        super().__init__()
        self.token_embedding = nn.Embedding(vocab_size, n_embed)
        self.blocks = nn.ModuleList([
            RoFormerBlock(n_embed, block_size, dropout, use_softmax=True, n_head=n_head, no_rope=no_rope)
    ...
    for it in range(start_iter, n_iters + 1):
        ...
        if it % eval_every == 0:
            ...
            with torch.no_grad():
                ...
                # Layout per level: table (3*n_keys+1) + Q section (3*n_keys+1) [Q key answer per entry + sep]
                # Q section key positions: q_start + 1 + 3*k for k in range(n_keys)
                n_keys = dataset.n_show
                n_hops = dataset.n_hops
                level_accs = []
                level_block = 3 * n_keys + 1 + 3 * n_keys + 1  # table + sep + Q section + sep
                for lev in range(n_hops):
                    # Q section starts after table + separator
                    q_section_start = lev * level_block + (3 * n_keys + 1)
                    # Key positions in Q section: q_section_start + 1 + 3*k (after each Q token)
                    level_positions = [q_section_start + 1 + 3 * k for k in range(n_keys)]
    ...
    for model_name in run_models:
        if model_name.startswith('N'):
            ...
            model = TransformerBaseline(
                ...
                n_head=args.n_head, no_rope=args.no_rope
            )
