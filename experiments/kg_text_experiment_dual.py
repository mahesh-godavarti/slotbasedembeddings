...
class KGDataset:
    ...
    def get_mlm_batch_native_slots(self, batch_size, device, mask_prob=0.15):
        ...
        Like get_mlm_batch_native but returns 4 values (no negate_angles),
        since J uses slot-based angles rather than cumsum direction.
...
class ModelJ(nn.Module):
    ...
    sequences are [head_chars][tail_chars] with NO relation token.
    ...
    KG: 2 slots with per-relation learned slot angles. Positions reset per slot.
        Bidirectional attention, MLM on character tokens only.
    ...
    def forward_kg(self, char_tokens, targets, head_lens, rel_names, negate_angles=None):
        ...
        negate_angles is accepted for API compatibility but ignored (angles come
        from slot assignments, not cumsum direction).
        """
        B, T = char_tokens.shape
        pad_mask = (char_tokens != 0)  # PAD = 0
...
class ModelK(nn.Module):
    ...
    def _add_slot_angles(self, slot_ids, rel_names, device, T):
        """Compute per-relation slot angle offsets.

        Args:
            slot_ids: (B, T) int — 0=HEAD, 1=TAIL
            rel_names: list of relation strings
            device: torch device
            T: sequence length

        Returns: (B, T, C//2) slot offsets
        """
        rel_indices = torch.tensor([self.rel_to_idx[r] for r in rel_names], device=device)  # (B,)
    ...
    def _kg_angles(self, raw_angles, head_lens, rel_names, device):
        ...
        # Per-relation slot angles
        slot_offset = self._add_slot_angles(slot_ids, rel_names, device, T)

        return raw_angles + rope + slot_offset

    def _kg_angles_slotcausal(self, raw_angles, slot_assignments, slot_positions,
                              rel_names, device):
        """Combine projected + RoPE + slot angles for rearranged slot-causal sequences.
        ...
        Returns: (B, T, C//2) = projected + slot_pos*freq + slot_offset
        """
        B, T, D = raw_angles.shape
        ...
        # Per-relation slot angles
        slot_offset = self._add_slot_angles(slot_assignments, rel_names, device, T)
    ...
    def forward_text_mlm(self, tokens, targets):
        """Text mode: bidirectional attention + RoPE, MLM on randomly masked tokens."""
        B, T = tokens.shape
        pad_mask = (tokens != 0)
        x = self.token_embedding(tokens)
        rope = self._rope_angles(T, tokens.device).expand(B, -1, -1)
    ...
    def forward_kg_causal(self, tokens, targets, slot_assignments, slot_positions,
        ...
        for l, block in enumerate(self.blocks):
            raw_angles = self.angle_projectors[l](x)  # (B, T, C//2)
            angles = self._kg_angles_slotcausal(raw_angles, slot_assignments,
                                                slot_positions, rel_names, tokens.device)
            x = block(x, angles, causal=False, pad_mask=pad_mask, attn_mask=attn_mask)
