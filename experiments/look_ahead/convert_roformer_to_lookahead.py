...
def convert(args):
    ...
    # Handle D=1 (d_block=1): look-ahead uses 'block.X' but roformer uses 'blocks.0.X'
    if args.d_block == 1:
        for name in list(la_state.keys()):
            if name.startswith('block.'):
                roformer_name = 'blocks.0.' + name[len('block.'):]
                if roformer_name in roformer_state:
                    assert la_state[name].shape == roformer_state[roformer_name].shape, \
                        f"Shape mismatch for {name}: {la_state[name].shape} vs {roformer_state[roformer_name].shape}"
                    la_state[name] = roformer_state[roformer_name]
