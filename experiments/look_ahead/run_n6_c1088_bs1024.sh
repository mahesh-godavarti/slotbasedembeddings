...
echo "$(date): Waiting for N=6 C=1088 bs512 to finish..."
while pgrep -f 'n_embed 1088.*n_layers 6.*block_size 512' > /dev/null 2>&1; do
    sleep 30
done
echo "$(date): Starting N=6 C=1088 bs1024 batch=32."

mkdir -p checkpoints_n6_c1088_bs1024 logs
...
/home/ubuntu/exp8/venv/bin/python train_wiki_streaming.py train \
    ...
    --gpu 0 \
    --amp 2>&1 | tee logs/roformer_n6_c1088_bs1024.log
echo "$(date): Finished N=6 C=1088 bs1024"
